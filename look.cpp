...
*/
 void scan(){
     ...
     if(direction == "Right"){
       for(int i = headPointer + 1; i <= maximum; i++ ){
          if(request.count(i)){
             headPointerAllocations.push_back(i);
             request.erase(i);
          }
       }
        headPointerAllocations.push_back(maximum);
       for(int i = headPointer - 1 ; i >= minimum; i--){
        ...
        headPointerAllocations.push_back(minimum);
        for(int i = headPointer + 1; i <= maximum; i++ ){
        if(request.count(i)){
             headPointerAllocations.push_back(i);
             request.erase(i);
